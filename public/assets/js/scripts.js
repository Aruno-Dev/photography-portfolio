...
//COMMENTS
    ...
    //display html in show comment badge
    $('.show').html('show comments');
    ...
    //Comments form validation functions
    function checkCommentEmail(input){
        var validEmail    = /^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/;
        return input.value.match(validEmail);
    }

    function checkCommentName(input){
    ...
    function checkCommentLength(input){
       minLength = 1;
       maxLength = 255;
       if(input.value.length >= 1 && input.value.length <= 255){
           return true;
       }else{
           return false;
       }
   ...
   function remainingCharacters(type){
    ...
    $(document).on('click','.post',function(){
        ...
        if(checkCommentEmail(emailInput)){
            $('.emailError').html('');
            if(checkCommentName(authorInput)){
                $('.nameError').html('');
                if(checkCommentLength(commentInput)){
    ...
    //disable slider while comments are opened
    ...
    function checkContactSubject(input){

    }

    function checkContactMessage(input){
